...
router.post("/", async (req, res) => {
  try {
    ...
    if (supabaseError) {
      ...
      return res.status(500).json({ message: "Failed to submit contact form", details: supabaseError.message });
    }
    ...
    res.status(201).json({ message: "Contact form submitted successfully", contact: data[0] });
  } catch (err) {
