...
const ParticleBackground = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (particles.length === 0 || Math.abs(canvas.width - window.innerWidth) > 100) {
        initializeParticles();
      }
    };
    ...
    class Particle {
      ...
      update() {
        ...
        if (distance < 100) {
          ...
          this.opacity = Math.min(this.baseOpacity * (this.isHovered ? 2 : 1.5), 1);
          ...
          if (distance < attractionRadius) {
            const forceMultiplier = (attractionRadius - distance) / attractionRadius;
      ...
      draw() {
        ...
        if (this.isHovered) {
          ...
          gradient.addColorStop(0.3, `rgba(99, 102, 241, ${pulseOpacity * 0.8})`);
          gradient.addColorStop(0.6, `rgba(147, 51, 234, ${pulseOpacity * 0.4})`); // Purple accent
          gradient.addColorStop(1, `rgba(59, 130, 246, 0)`);
        } else {
          gradient.addColorStop(0, `rgba(59, 130, 246, ${pulseOpacity})`);
          gradient.addColorStop(0.5, `rgba(99, 102, 241, ${pulseOpacity * 0.6})`);
          gradient.addColorStop(1, `rgba(59, 130, 246, 0)`);
        }
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < maxDistance) {
            ...
            if (particles[i].isHovered || particles[j].isHovered) {
              ...
              gradient.addColorStop(0.3, `rgba(99, 102, 241, ${opacity * 0.9})`);
              gradient.addColorStop(0.7, `rgba(147, 51, 234, ${opacity * 0.7})`);
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      rippleRef.current.push(new (class Ripple {
        constructor(x, y) {
          this.x = x;
          this.y = y;
          this.radius = 0;
